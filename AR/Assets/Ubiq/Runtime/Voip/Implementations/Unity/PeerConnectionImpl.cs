    ...
    public class PeerConnectionImpl : IPeerConnectionImpl
    {
            ...
            bool polite, List<IceServerDetails> iceServers)
        {
            ...
            receiverStream.OnAddTrack += (MediaStreamTrackEvent e) =>
            {
                ...
                {
                    // Here is where the video stream is received
                    var track = e.Track as VideoStreamTrack;
                    var go = GameObject.CreatePrimitive(PrimitiveType.Quad);
                    var screenanchor = GameObject.Find("StreamAnchor");
                    go.transform.SetParent(screenanchor.transform,false);
                    this.mat = go.GetComponent<Renderer>().material;
                    track.OnVideoReceived += tex =>
                    {
                        // Use this event to drop the texture onto
                        // something
        ...
        private IEnumerator StartMicrophoneTrack()
        {
            ...
            //var cam = Camera.main;// will froze phone screen, but reciever screen works fine.
            //videoStreamTrack = cam.CaptureStreamTrack(256,256);

            //Graphics.Blit(null, renderTexture, m_ARCameraBackground.material);

            RenderTexture renderTexture = Resources.Load<RenderTexture>("TargetTexure");
